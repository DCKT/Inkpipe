...
export async function runPipeline(result: ProwlarrResult): Promise<void> {
  ...
  try {
    ...
    magnetId = upload.id
    console.log(`[pipeline] [job ${job.id}] Magnet uploaded, id: ${upload.id}, ready: ${upload.ready}`)
    ...
    if (!upload.ready) {
      ...
      while (true) {
        ...
        if (status.ready) {
          console.log(`[pipeline] [job ${job.id}] Debrid ready after ${pollCount} polls`)
          break
        }
        if (status.statusCode >= 5) {
          throw new Error(`AllDebrid magnet error: ${status.status} (code ${status.statusCode})`)
        }
        console.log(`[pipeline] [job ${job.id}] Debrid not ready (poll #${pollCount}, status: ${status.status}), waiting...`)
        await sleep(POLL_INTERVAL)
      }
    } else {
      console.log(`[pipeline] [job ${job.id}] Already ready at upload, skipping poll`)
    }

    // Stage 2b: Fetch file links via v4/magnet/files
    const debridFiles = await getMagnetFiles(upload.id, config)
    console.log(`[pipeline] [job ${job.id}] Got ${debridFiles.length} files from AllDebrid`)
    ...
    console.log(`[pipeline] [job ${job.id}] Stage: DOWNLOADING (${debridFiles.length} files)`)
    ...
    for (let i = 0; i < debridFiles.length; i++) {
      ...
      console.log(`[pipeline] [job ${job.id}] Unlocking file ${i + 1}/${debridFiles.length}: ${file.filename}`)
      const unlocked = await unlockLink(file.link, config)
      const destPath = join(jobDir, unlocked.filename)
      console.log(`[pipeline] [job ${job.id}] Downloading to: ${destPath} (${unlocked.size} bytes)`)
    ...
    if (!epubFile) {
      ...
      if (cbzFile) {
        console.log(`[pipeline] [job ${job.id}] Converting: ${cbzFile}`)
        await convertWithKcc(cbzFile, jobDir, config)
        console.log(`[pipeline] [job ${job.id}] Conversion complete`)
      } else {
        console.log(`[pipeline] [job ${job.id}] No convertible file found, skipping conversion`)
      }
    } else {
      console.log(`[pipeline] [job ${job.id}] Already an EPUB, skipping conversion: ${epubFile}`)
    }
    ...
    if (config.copyparty.url) {
      ...
      if (finalFile) {
        console.log(`[pipeline] [job ${job.id}] Uploading to Copyparty: ${finalFile}`)
        await uploadToCopyparty(finalFile, config)
        console.log(`[pipeline] [job ${job.id}] Copyparty upload complete`)
      } else {
        console.log(`[pipeline] [job ${job.id}] No file found to upload to Copyparty`)
      }
    } else {
      console.log(`[pipeline] [job ${job.id}] Copyparty not configured, skipping upload`)
    }
