...
const wrapSqlError =
  ...
  <A, E, R>(
    ...
    Effect.mapError(
      effect,
      (error) => new PersistenceError({ operation, cause: error }),
    );
```
...
yield *
  auditLogService.log(entry).pipe(
...
yield *
  auditLogService
    .log(entry)
    .pipe(Effect.mapError((e) => new MyError({ cause: e })));
...
const result = effect
  .pipe(Effect.map(transformA), Effect.flatMap(fetchRelated))
  .pipe(
...
```typescript
export interface TreeNodeEncoded extends Schema.Schema.Encoded<
  typeof TreeNode
> {}
...
export class TreeNode extends Schema.TaggedClass<TreeNode>()("TreeNode", {
  ...
  children: Schema.Array(
    Schema.suspend((): Schema.Schema<TreeNode, TreeNodeEncoded> => TreeNode),
  ),
}) {}
```
...
// In Schema classes
export class AccountHierarchy extends Schema.Class<AccountHierarchy>(
  "AccountHierarchy",
)({
  root: Account,
...
const rows =
  yield * sql<{ count: string }>`SELECT COUNT(*) as count FROM accounts`;
...
const findByCompany = SqlSchema.findAll({
  ...
  Result: AccountSchema,
  execute: (companyId) =>
    sql`SELECT * FROM accounts WHERE company_id = ${companyId}`,
});
...
it.prop(
  "symmetry",
  [Schema.Number, FastCheck.integer()],
  ([a, b]) => a + b === b + a,
);
